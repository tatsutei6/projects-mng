...
const CreateKanban = (props: { projectId: number | string, setKanbans: Dispatch<SetStateAction<Kanban[]>> }) => {
  ...
  const submit = () => {
    ...
    (async () => {
      ...
      console.log('ProjectPage useEffect.value', kanbans)
    ...
    // Dao.getInstance().saveKanban(props.projectId, name).then((value1) => {
    //   Dao.getInstance().getKanbansByProjectId(Number(props.projectId)).then((value2) => {
    //     props.setKanbans(value2)
    //     console.log('ProjectPage useEffect.value', value2)
    //   })
    //   setName('')
    // })
  }
